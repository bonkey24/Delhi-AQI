...
print("==================================================")
print("  XGBOOST AQI PREDICTION (STRICT NON-LEAKING)")
print("==================================================")
...
print("\n[Step 1.5] Fixing Temporal Data Leakage...")
leaking_features = ['AQI_roll_mean_7', 'AQI_ewma_7', 'AQI_diff_1']
print(f"   Shifting columns to prevent cheating: {leaking_features}")

for col in leaking_features:
    df[col] = df[col].shift(1)

# Drop the new NaN row created by the shift
...
print("\n[Step 4] Training XGBoost Regressor (Tuned for Real-World Accuracy)...")
...
ax.set_title(f"XGBoost: Actual vs Predicted AQI (Unbiased)\nTest MAE: {test_mae:.1f} | Test R²: {test_r2:.3f}", fontsize=14, pad=12)
ax.set_ylabel("Air Quality Index (AQI)")
ax.legend()
plt.xticks(rotation=30)
plt.tight_layout()
plt.savefig("xgb_actual_vs_predicted_fixed.png", dpi=150, facecolor=fig.get_facecolor())
plt.close()
print("   * Saved: xgb_actual_vs_predicted_fixed.png")

# Feature Importance Plot
fig, ax = plt.subplots(figsize=(10, 6))
ax.barh(imp_df['Feature'][::-1], imp_df['Importance'][::-1], color="#ce93d8")
ax.set_title("XGBoost Feature Importance (Unbiased Top 10)", fontsize=14, pad=12)
ax.set_xlabel("Relative Importance")
plt.tight_layout()
plt.savefig("xgb_feature_importance_fixed.png", dpi=150, facecolor=fig.get_facecolor())
plt.close()
print("   * Saved: xgb_feature_importance_fixed.png")

print("\n==================================================")
print("  * STRICT ML PIPELINE COMPLETE")
print("==================================================")
